...
const renderOptions = (data, containerEl, type) => {
    ...
    const items = data[type]; // Get the list of items (e.g., BREAD_DATA.base)
    ...
    items.forEach(item => {
        ...
        containerEl.appendChild(createOptionCard(item, type, isSelected)); // Append card to the container
    });
};
...
const initializeBuilder = () => {
    // ตรวจสอบว่า BREAD_DATA ถูกโหลดสำเร็จหรือไม่
    if (!window.BREAD_DATA || !window.BREAD_DATA.base) {
         console.error("BREAD_DATA failed to load or is empty.");
         return; // หยุดการทำงานถ้าข้อมูลหายไป
    }
    // These lines must execute successfully!
    renderOptions(window.BREAD_DATA, baseOptionsEl, 'base'); 
    renderOptions(window.BREAD_DATA, mixinOptionsEl, 'mixins');
    renderOptions(window.BREAD_DATA, toppingOptionsEl, 'toppings');
    updatePreview();
    calculatePrice();
    updateCartDisplay();
};

document.addEventListener('DOMContentLoaded', initializeBuilder); // Function is called when the page loads
